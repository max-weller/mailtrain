...
module.exports.updateMessage = (message, status, failureSeverity, callback, responseText) => {
    db.getConnection((err, connection) => {
        if (err) {
            return callback(err);
        }
        ...
        if (status === 'unsubscribed') {
            ...
            failureSeverity = 1000;
        } else if (status === 'bounced') {
            statusCode = subscriptions.Status.BOUNCED;
            if (failureSeverity === true) failureSeverity = 10;
            if (failureSeverity === false) failureSeverity = 0;
        ...
        log.verbose("updateMessage", "responseText: ",responseText);

        let query = 'UPDATE `campaigns` SET `' + status + '`=`' + status + '`+1 WHERE id=? LIMIT 1';
        connection.query(query, [message.campaign], () => {

            let query = 'UPDATE `campaign__' + message.campaign + '` SET status=?, updated=NOW(), response=CONCAT(response, ?) WHERE id=? LIMIT 1';
            connection.query(query, [statusCode, responseText, message.id], err => {
                connection.release();
                if (err) {
                    return callback(err);
                }
                ...
                if (updateSubscription) {
                    subscriptions.changeStatus(message.list,
                                               message.subscription,
                              /* campaignId */ statusCode === subscriptions.Status.UNSUBSCRIBED ? message.campaign : false,
                   /* new subscriber status */ statusCode,
                                               callback,
                    /* status change reason */ responseText,
                                               failureSeverity);
                } else {
                    return callback(null, true);
                }
