...
module.exports.changeStatus = (listId, id, campaignId, status, callback, reason) => {
    db.getConnection((err, connection) => {
        if (err) {
            return callback(err);
        }
        connection.beginTransaction(err => {
            ...
            connection.query('SELECT `status` FROM `subscription__' + listId + '` WHERE id=? LIMIT 1', [id], (err, rows) => {
                ...
                connection.query('UPDATE `subscription__' + listId + '` SET `status`=?, `status_change`=NOW(), `status_change_reason` = ? WHERE id=? LIMIT 1', [status, reason, id], err => {
                    if (err) {
                        return helpers.rollbackAndReleaseConnection(connection, () => callback(err));
                    }
