...
module.exports.changeStatus = (listId, id, campaignId, status, callback, reason, severity) => {
    db.getConnection((err, connection) => {
        if (err) {
            return callback(err);
        }
        connection.beginTransaction(err => {
            ...
            connection.query('SELECT `status`, `error_level` FROM `subscription__' + listId + '` WHERE id=? LIMIT 1', [id], (err, rows) => {
                if (err) {
                    return helpers.rollbackAndReleaseConnection(connection, () => callback(err));
                }
                ...
                var error_level = rows[0].error_level + severity
                if (error_level < 10) {
                    statusChange = false;
                    status = oldStatus;
                }
                ...
                if (statusChange && (oldStatus === Status.SUBSCRIBED || status === Status.SUBSCRIBED)) {
                    // should we add or subtract from the subscriber count?
                    statusDirection = status === Status.SUBSCRIBED ? '+' : '-';
                }

                connection.query('UPDATE `subscription__' + listId + '` SET `status`=?, `error_level`=?, `status_change`=NOW(), `status_change_reason` = ? WHERE id=? LIMIT 1', [status, error_level, reason, id], err => {
                    if (err) {
                        return helpers.rollbackAndReleaseConnection(connection, () => callback(err));
                    }
